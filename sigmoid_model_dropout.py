'''
Sigmoid Activation function model with Dropout regularization

This model implements a fully connected neural network with 2 hidden layers.
Dropout regularization is included to reduce overfitting.
It uses:
...
- Dropout regularization (20%) in both hidden layers
'''
...
# Define model with Sigmoid and Dropout
model = Sequential([
    Dense(128, activation='sigmoid', input_shape=(X_train.shape[1],)),  # First hidden layer
    Dropout(0.2),  # Dropout (20%) to reduce overfitting
